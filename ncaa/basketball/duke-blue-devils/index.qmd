...
duke <- read_csv("../../../pipeline/ncaa/basketball/data/team_stats/team_Duke_2026.csv",
...
> See also: [Team Season Grade Report](team-grade.html) — category grades, scoring trend, and a logistic regression look at whether field goal % predicts wins.
...
## Shooting Efficiency Over the Season (Animated) {#animated-scatter}
...
fit <- lm(team_score ~ field_goal_pct, data = duke_anim)
line_df <- data.frame(
  field_goal_pct = range(duke_anim$field_goal_pct)
) |>
  mutate(team_score = predict(fit, newdata = pick(everything())))

p <- ggplot(duke_anim, aes(x = field_goal_pct, y = team_score)) +
  geom_point(color = duke_blue, size = 2.5, alpha = 0.7) +
...
anim <- animate(
  p,
  nframes  = min(max(duke_anim$game_num), 60),
  fps      = 6,
  width    = 900,
  height   = 650,
...
anim_save("duke_fg_pct_vs_score.gif", animation = anim)
```

![](duke_fg_pct_vs_score.gif)
